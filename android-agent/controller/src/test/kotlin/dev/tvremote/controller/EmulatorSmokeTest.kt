...
import dev.tvremote.controller.net.WsDebugClient
...
class EmulatorSmokeTest {
    ...
    private val agentComponent = "dev.tvremote.agent.debug/dev.tvremote.agent.service.AgentService"
    ...
    fun fullChainPairingAuthenticationControl() {
        ...
        adbShell("pm", "clear", "dev.tvremote.agent.debug")
        Thread.sleep(2000)
        ...
        adbShell("am", "startservice",
            "-n", "dev.tvremote.agent.debug/dev.tvremote.agent.service.AgentService")
        Thread.sleep(4000)
        adbShell("am", "start", "-n", "dev.tvremote.agent.debug/dev.tvremote.agent.MainActivity")
        Thread.sleep(3000)
    ...
    private fun openPairingWindow(): String {
        ...
        adbShell(
            "am", "startservice", "-a", "dev.tvremote.agent.OPEN_PAIRING",
            "-n", "dev.tvremote.agent.debug/dev.tvremote.agent.service.AgentService",
        )
        Thread.sleep(3000)
        repeat(10) {
    ...
    private fun restoreAccessibility() {
        ...
        adbShell(
            ...
            "dev.tvremote.agent.debug/dev.tvremote.agent.service.TvAccessibilityService",
        )
        Thread.sleep(6000)
    }
}
