...
class PairingManager(
    ...
    private data class Pending(
        ...
        /** SAS 确认阶段的独立截止时间，避免配对窗口到期把待确认会话一起清掉。 */
        var sasExpiresAtMs: Long = 0,
    ...
    private fun beginPairing(
        ...
        // 进入 SAS 后给出独立的确认时限：扫/输码可能已用掉窗口大半时间，
        // 若仍以窗口到期为准，电视端确认弹窗会瞬间消失，用户来不及核对。
        val sasDeadline = maxOf(current.window.expiresAtMs, nowMs() + SAS_CONFIRM_TTL_MS)
        current.sasExpiresAtMs = sasDeadline
    ...
    fun awaitDecision(pairingId: String, timeoutMs: Long): PairingDecision? {
        val snapshot = synchronized(this) {
            ...
            val deadline = maxOf(current.window.expiresAtMs, current.sasExpiresAtMs)
            current to maxOf(0L, minOf(timeoutMs, deadline - nowMs()))
        }
    ...
    companion object {
        ...
        /** SAS 确认阶段的独立时限（自提交配对码/扫码起算），保证电视端确认弹窗有足够时间。 */
        const val SAS_CONFIRM_TTL_MS = 120_000L
        // 二维码与配对窗口同寿：窗口开启期间二维码始终有效，避免窗口后半段退化为仅数字码。
        const val QR_TOKEN_TTL_MS = PAIRING_TTL_MS
