...
class PairingManagerTest {
    ...
    @Test
    fun sasConfirmationSurvivesWindowExpiry() {
        var now = 1_000L
        val manager = PairingManager(nowMs = { now }, random = DeterministicRandom())
        val window = manager.openWindow()
        // 在窗口后段提交：SAS 应拿到独立的确认时限，而不是跟着窗口一起到期
        now = window.expiresAtMs - 10_000L
        val submission = manager.submit(
            window.code, "Controller", ByteArray(32), ByteArray(32),
        ) as PairingSubmission.AwaitingTvConfirmation
        assertTrue(submission.details.expiresAtMs > window.expiresAtMs)

        now = window.expiresAtMs + 1
        assertNotNull(manager.confirm(submission.details.pairingId, true))
        assertTrue(manager.awaitDecision(submission.details.pairingId, 1) is PairingDecision.Accepted)
    }
