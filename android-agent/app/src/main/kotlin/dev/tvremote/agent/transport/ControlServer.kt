...
import dev.tvremote.agent.model.TextAction
import dev.tvremote.agent.model.TextCommand
...
import dev.tvremote.agent.protocol.ProtocolException
...
import dev.tvremote.agent.protocol.optionalString
...
class ControlServer(
    ...
    private fun handleKeyEvent(
        ...
        val ack = if (!limiter.allow()) {
            dev.tvremote.agent.model.CommandAck(message.sequence, AckStatus.REJECTED, "rate limit exceeded")
        } else {
            ...
            if (key == null || state == null || repeatCount !in 0..MAX_REPEAT_COUNT.toLong()) {
                dev.tvremote.agent.model.CommandAck(message.sequence, AckStatus.REJECTED, "invalid key event")
            } else {
    ...
    private fun handleTextCommand(
        ...
        val ack = if (!limiter.allow()) {
            ...
            if (text.isEmpty()) {
                dev.tvremote.agent.model.CommandAck(message.sequence, AckStatus.REJECTED, "text must not be empty")
            } else {
