...
import dev.lucasdone.tvremote.xiaomi.backend.*
...
class BackendTest {
    ...
    ) : dev.lucasdone.tvremote.agent.command.CommandExecutor {
    ...
    @Test fun adbRequiresExplicitOptInAndXiaomiHasPriority() {
        ...
        assertEquals(BackendKind.XIAOMI, BackendSelection.select(true, true, true))
    ...
    @Test fun xiaomiLoopbackIsTheDefaultPreferredProvider() {
        assertEquals("xiaomi-loopback", LocalBackends.defaults().first().id)
    }
    ...
    @Test fun failedBackendDegradesToUnsupportedAndNeverReportsSupported() {
        ...
        assertEquals("BEST_EFFORT", TvCapabilities.keyState(LogicalKey.DPAD_UP, live, false))
        assertEquals("UNSUPPORTED", TvCapabilities.keyState(LogicalKey.DPAD_UP, dead, false))
        assertEquals("UNSUPPORTED", TvCapabilities.keyState(LogicalKey.DPAD_UP, null, false))
        assertEquals("BEST_EFFORT", TvCapabilities.keyState(LogicalKey.VOLUME_UP, null, false))
        assertEquals("UNSUPPORTED", TvCapabilities.keyState(LogicalKey.VOLUME_UP, null, true))
        assertEquals("BEST_EFFORT", TvCapabilities.keyState(LogicalKey.MEDIA_NEXT, null, false))
        for (key in LogicalKey.values())
            for (backend in listOf(null, live, dead))
                assertNotEquals("SUPPORTED", TvCapabilities.keyState(key, backend, false))
    }
}
