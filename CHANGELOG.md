...
- Hardened the Rust WS core and the debug-channel clients (external audit follow-up): the Rust `WsDecoder` now keeps parse state until a full frame (header/extended length/mask/payload) has arrived and drains every complete message from one read; FFI `WsCodec.push` returns all complete frames and the iOS client queues them instead of keeping only the first (and no longer pre-reads a 2-byte header); Kotlin/TS/iOS heartbeats now share the strictly increasing command sequence (previously `sequence=0`, which the agent rejects — the Kotlin heartbeat failed `ProtocolCodec.encode` and closed the connection every 15s). `SessionCrypto` now rejects `replay_window_bits` outside `1..=64` with `Invalid`, `SessionKeys` no longer derives `Debug` and zeroizes on drop, `DirectionCipher`/PSK copies use `Zeroizing`, and the `aes` `zeroize` feature is enabled. Known limitation: `polyval` 0.6.2 still marks its pmull-backend zeroize as TODO, so no claim is made that all GHASH/AES-GCM backend memory is cleared.
